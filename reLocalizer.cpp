...

{
	...
	string sFileName_mov = sDataRoot + "obs_000000.ply"; 
	pcl::PointCloud<pcl::PointNormal>::Ptr cloud_mov(new pcl::PointCloud<pcl::PointNormal>);
	plyReader.read(sFileName_mov, *cloud_mov);
	CFPFH MovFPFH; 
	MovFPFH.setInputCloud(cloud_mov); 
	MovFPFH.compute(); 
	
	string sFileName_ref = sDataRoot + "obs_000005.ply";
	pcl::PointCloud<pcl::PointNormal>::Ptr cloud_ref(new pcl::PointCloud<pcl::PointNormal>);
	...
	pcdWriter.write<pcl::PointNormal>(cDataDir, *cloud_mov);
	...
	pcdWriter.write<pcl::PointNormal>(cDataDir, *cloud_ref);
