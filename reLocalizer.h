...
class CFPFH {
 public:
  ...
  void decoupleCloud(
      const pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_normals,
  ...
  void setInputCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud) {
    ...
    pcl::copyPointCloud(*input_cloud, *cloud_tmp);
    m_cloud.swap(cloud_tmp);
    ...
    computeNormals(m_cloud, m_searchMethod, m_cloud_normal);
  }
  void setInputCloud(pcl::PointCloud<pcl::PointNormal>::Ptr input_cloud) {
    decoupleCloud(input_cloud, m_cloud, m_cloud_normal);
    pcl::search::KdTree<pcl::PointXYZ>::Ptr search_method(
        new pcl::search::KdTree<pcl::PointXYZ>);
    search_method->setInputCloud(m_cloud);
    m_searchMethod.swap(search_method);
  ...
  void computeKeypoints() {
    ...
    for (int i = 0; i < nLen; i++) {
      ...
      m_searchMethod->nearestKSearch(searchPoint, 1, pointIdxNKNSearch,
  ...
  void computeFPFH() {
    ...
    fpfh_estimation->setInputNormals(m_cloud_normal);
...
Eigen::Matrix4f EstimateCorrespondence(CFPFH& MovFPFH, CFPFH& RefFPFH,
                                       double dDistThr) {
  ...
  rejector_sac.setInlierThreshold(
      dDistThr);  // the unit of distance is meter, not the squared distance,
                  // original is 2.5
  ...
  printf("Determine correspondences via ransac: %03d vs %03d\n",
         static_cast<int>(corres->size()),
         static_cast<int>(correspondences_filtered->size()));
  //// refined results.
  ...
  transformation_estimation.estimateRigidTransformation(
      *MovFPFH.m_cloud_keypoints, *RefFPFH.m_cloud_keypoints, *corres, Tf0);
